# TODO: I adapted this from a GitHub repo. To link source here. -- Georgios Rizos
...
class SpecAugment:
    ...
    def __init__(self, mel_spectrogram):
        ...
        self.W, self.F, self.m_F, self.T, self.m_T = None, 16, 2, 24, 2
    ...
    def freq_mask(self):
        ...
        if len(self.mel_spectrogram.shape) == 3:
            for i in range(self.m_F):
                f = int(np.random.uniform(0, self.F))  # [0, F)
                f0 = random.randint(0, v - f)  # [0, v - f)
                self.mel_spectrogram[:, :, f0:f0 + f] = 0
        elif len(self.mel_spectrogram.shape) == 4:
            for c in range(self.mel_spectrogram.shape[3]):
                for i in range(self.m_F):
                    f = int(np.random.uniform(0, self.F))  # [0, F)
                    f0 = random.randint(0, v - f)  # [0, v - f)
                    self.mel_spectrogram[:, :, f0:f0 + f, c] = 0
        else:
        ...
        if len(self.mel_spectrogram.shape) == 3:
            ...
            for c in range(self.mel_spectrogram.shape[3]):
                for i in range(self.m_T):
                    t = int(np.random.uniform(0, self.T))  # [0, T)
                    t0 = random.randint(0, tau - t)  # [0, tau - t)
                    self.mel_spectrogram[:, t0:t0 + t, :, c] = 0
        else:
            raise NotImplementedError
